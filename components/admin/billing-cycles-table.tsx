...
const paymentBadge: Record<string, string> = {
  ...
  isento:    'bg-slate-100 text-slate-500 border-slate-200',
}
const paymentLabel: Record<string, string> = {
  pendente: 'Pendente', pago: 'Pago', em_atraso: 'Em Atraso', isento: 'Isento',
}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/60">
              {([
                { label: 'Ciclo', hide: false },
                { label: 'Período', hide: false },
                { label: 'Estado', hide: false },
                { label: 'Dias Paus.', hide: true },
                { label: 'Pessoas', hide: false },
                { label: 'Valor', hide: false },
                { label: 'Pagamento', hide: false },
                { label: 'Isento', hide: true },
                { label: 'Fatura AT', hide: true },
              ] as { label: string; hide: boolean }[]).map(h => (
                <th key={h.label} className={cn('text-left px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap', h.hide && 'nc-col-hide-mobile')}>
                  {h.label}
                </th>
              ))}
            </tr>
            ...
            {cycles.map(cycle => {
              ...
              return (
                <tr key={cycle.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="nc-col-hide-mobile px-4 py-3 text-slate-500">
                  ...
                  <td className="nc-col-hide-mobile px-4 py-3">
                  ...
                  <td className="nc-col-hide-mobile px-4 py-3 text-slate-500 font-mono text-xs">
