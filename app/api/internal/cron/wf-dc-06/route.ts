...
// WF-DC-06 — Mid-Cycle Email (Day 15)
// Runs daily at 12:00 UTC. Finds active cycles that started exactly 15 days ago
// and whose mid-cycle email has not yet been sent, then sends it via Resend.
export async function POST(request: Request) {
  ...
  if (error) {
    console.error('[wf-dc-06] fetch error:', error)
  ...
  for (const cycle of cycles) {
    ...
    try {
      ...
      await sendSlackMessage({
        channel: 'clientes',
        text: `📧 Email mid-cycle enviado — ${restaurant.nome} → ${client.email_faturacao}`,
      })

      results.push({ restaurant: restaurant.nome, email: client.email_faturacao, status: 'sent' })
    } catch (err) {
      console.error(`[wf-dc-06] error for ${restaurant.nome}:`, err)
